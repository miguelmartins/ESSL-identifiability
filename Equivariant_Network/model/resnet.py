...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_block, dropout=0., num_classes=100, head='linear'):
        super().__init__()
        ...
        if head == 'linear':
            self.projector = nn.Linear(512 * block.expansion, 4)
        elif head == 'mlp':
    ...
    def forward(self, x, rot_pred=False):
        output = self.conv1(x)
        ...
        z = output.view(output.size(0), -1)
        if rot_pred:
            output = self.fc(z.detach())
            return output, self.projector(z)
        else:
            output = self.fc(z)
            return output
...
class C4ResNet(nn.Module):
    ...
    def __init__(self, block, num_block, kernel=3, reduction=2, groups=2, dropout=0., num_classes=100, head='linear'):
        super().__init__()
        ...
        if head == 'linear':
            self.projector = nn.Linear(256 * 16, 4)
        elif head == 'mlp':
            self.projector = nn.Sequential(nn.Linear(256 * 16, 2048),
                                           nn.ReLU(),
                                           nn.Linear(2048, 4))
        else:
...
def resnet18(dropout, num_classes, head='linear'):
    """ return a ResNet18 object
    """
    return ResNet(BasicBlock, [2, 2, 2, 2], dropout, num_classes=num_classes, head=head)


def C4resnet18(dropout, num_classes, head='linear'):
    """ return a C4ResNet18 object
    """
    return C4ResNet(C4Basic, [2, 2, 2, 2], dropout=dropout, num_classes=num_classes, head=head)
